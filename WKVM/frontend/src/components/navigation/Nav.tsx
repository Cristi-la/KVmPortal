...
function NavLink({
    ...
    return (
        ...
        className={cn(
            ...
            'h-10 justify-start text-wrap rounded-none px-6',
        ...
        {title}
        {label && <div className='ml-2 rounded-lg bg-primary px-1 text-[0.7rem] text-primary-foreground'>{label}</div>}
        </Link>
    )
}
...
return (
    ...
    <CollapsibleTrigger
        ...
        'group h-10 w-full justify-start rounded-none px-6'
        )}
    >
        <div className='mr-2'>{icon}</div>
        {title}
  ...
  return (
    <Tooltip delayDuration={0}>
      ...
      <TooltipContent side='right' className='flex items-center gap-4'>
        {title}{label && <div className='rounded-lg bg-primary px-1 text-[0.7rem] text-primary-foreground'>{label}</div>}
      </TooltipContent>
    </Tooltip>
  )
}
...
const DocLink: React.FC<LinkProps> = ({children, ...props}) => {
 return (
  <Link className='font-medium text-primary hover:underline italic' {...props}>
    {children}
  </Link>
 )
}
